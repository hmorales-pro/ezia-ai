...
// GET /api/me/business/[businessId]/ezia-memory - Récupérer la mémoire d'Ezia
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ businessId: string }> }
) {
  console.log("[EziaMemory API] GET request received");
  
  try {
    const params = await context.params;
    const { businessId } = params;
    console.log("[EziaMemory API] BusinessId:", businessId);
    
    // Vérifier l'authentification
    const user = await isAuthenticated();
    if (user instanceof NextResponse || !user) {
      console.log("[EziaMemory API] Unauthorized");
      return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
    }
    
    const userId = user.id;
    console.log("[EziaMemory API] UserId:", userId);
    ...
    // TODO: Réactiver la génération du résumé exécutif après débogage
    // Commenté temporairement pour isoler le problème
    /*
    if (business.market_analysis && business.competitor_analysis) {
      try {
        ...
        eziaMemory.executive_summary = executiveSummary;
      } catch (error) {
...
// POST /api/me/business/[businessId]/ezia-memory/search - Rechercher dans la mémoire
